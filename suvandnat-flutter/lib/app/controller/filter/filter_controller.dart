...
class FilterController extends GetxController {
  ...
  Future<void> getFilterList() async {
    try {
      ...
      if (data.isNotEmpty == true) {
        ...
        selectedCategory.value = FilterModel.fromJson(filterList[0] as Map<String, dynamic>);
  ...
  void searchFilter(String itemTitle) {
    ...
    if (itemTitle.isNotEmpty) {
      ...
      debugPrint("Results: ${results.length.toString()} ${subCategoryList?.length.toString()}");
    } else {
      for (int i = 0; i < (filterModelList?.length ?? 0); i++) {
        if (i == currentCategoryIndex.value) {
