...
import 'package:meta_package/api/models/checkout_order/multi_address_model.dart' hide Region;
import 'package:meta_package/api/models/checkout_order/multi_address_model.dart' as multi_address hide Region;
...
class CheckoutOrderController extends GetxController {
  ...
  RxList<EstimateShippingMethodModel> estimatesList = <EstimateShippingMethodModel>[].obs;
  ...
  CheckOutOrderAddressModel checkOutOrderAddressModel = CheckOutOrderAddressModel();
  ...
  RxList<bool> validatorList = List
      .generate(11, (index) => false)
      .obs;
  ...
  Future<void> billingAddressOnTap(int index,
      multi_address.Address? address) async {
  ...
  Future<void> getGuestEstimateAndShipInformationFromApi({String paramShipping = '', String paramBilling = ''}) async {
    ...
    try {
      ...
      List<EstimateShippingMethodModel> data = await checkoutOrderAPIRepository.postGuestEstimateAPIResponse(params);
      ...
      ShippingInformationModel data1 = await checkoutOrderAPIRepository.postGuestShippingInformationResponse(params1);
  ...
  Future<void> getGuestAddress() async {
    ...
    if (validation()) {
      if (value.isNotEmpty) {
        checkOutOrderAddressModel = CheckOutOrderAddressModel.fromJson(jsonDecode(value) as Map<String, dynamic>);
        ...
        countryName.value = checkOutOrderAddressModel.countryName?.fullNameEnglish ?? '';
        ...
        countryNameBilling.value = checkOutOrderAddressModel.countryNameBilling?.fullNameEnglish ?? '';
        ...
        shippingCarrierCode.value = checkOutOrderAddressModel.shippingCarrierCode ?? 'freeshipping';
        shippingMethodCode.value = checkOutOrderAddressModel.shippingMethodCode ?? 'freeshipping';

        selectedShippingIndex.value = shippingCarrierCode.value == 'freeshipping' ? 0 : 1;
        ...
        selectedCoutry1.value = checkOutOrderAddressModel.countryName ?? CountryListModel();
        selectedStateList1.value = selectedCoutry1.value.availableRegions ?? [];
        selectedState1.value = checkOutOrderAddressModel.state ?? AvailableRegion();
        ...
        billingFirstNameController.text = checkOutOrderAddressModel.firstNameBilling ?? '';
        billingLastNameController.text = checkOutOrderAddressModel.lastNameBilling ?? '';
        billingEmailController.text = checkOutOrderAddressModel.emailBilling ?? '';
        billingAddress1Controller.text = checkOutOrderAddressModel.add1Billing ?? '';
        billingAddress2Controller.text = checkOutOrderAddressModel.add2Billing ?? '';
        billingAddress3Controller.text = checkOutOrderAddressModel.add3Billing ?? '';
        billingCityController.text = checkOutOrderAddressModel.cityBilling ?? '';
        selectedCoutry2.value = checkOutOrderAddressModel.countryName ?? CountryListModel();
        selectedState2.value = checkOutOrderAddressModel.state ?? AvailableRegion();
        billingStateController.text = checkOutOrderAddressModel.stateBilling?.name ?? '';
        billingZipPovinceController.text = checkOutOrderAddressModel.zipCodeBilling ?? '';
        billingPhoneNumberController.text = checkOutOrderAddressModel.phoneBilling ?? '';
  ...
  Future<void> setAddress() async {
    checkOutOrderAddressModel = CheckOutOrderAddressModel(
      ...
      shippingCarrierCode: (estimatesList.isEmpty)
          ...
          : estimatesList[selectedShippingIndex.value].carrierCode ?? 'freeshipping',
      shippingMethodCode: (estimatesList.isEmpty)
          ? shippingMethodCode.value
          : estimatesList[selectedShippingIndex.value].methodCode ?? 'freeshipping',
    );
    await LocalStore.setSavedGuestAddress(jsonEncode(checkOutOrderAddressModel.toJson()));
  }
  ...
  Future<void> myBillingAddress() async {
    ...
    shippingAddress = multiAddressModel?.value.addresses?[selectedAddressIndex.value] ?? multi_address.Address();

    if (isSameAsBilling.value) {
      billingAddress = multiAddressModel?.value.addresses![selectedAddressIndex.value] ?? multi_address.Address();
      await estimateAndShippingAPICall(shippingAddress, billingAddress);
    } else {
      billingAddress = multiAddressModel?.value.addresses![selectedBillingIndex.value] ?? multi_address.Address();
      await estimateAndShippingAPICall(shippingAddress, billingAddress);
    }
    checkEnablePlaceOrder();
  }
  ...
  Future<void> getEstimateAndShipInformationFromApi() async {
    try {
      isLoading.value = true;
      MultiAddressModel? data = await addressListAPIRepository.getMultiAddressAPIResponse();
      log('getAddressList 000 :-> $data');
      if (data != null) {
        multiAddressModel?.value = data;
        billingMultiAddressModel?.value = data;
        log('billingMultiAddressModel?.value is:--->>> ${multiAddressModel
            ?.value.addresses}');
        if (multiAddressModel != null) {
          if (multiAddressModel?.value.addresses?.isNotEmpty ?? false) {
            ...
            if (multiAddressModel?.value.addresses?.isNotEmpty ?? false) {
              shippingAddress = multiAddressModel?.value.addresses?.first ?? multi_address.Address();
            }
          }
        }
        checkEnablePlaceOrder();
  ...
  Future<void> estimateAndShippingAPICall(multi_address.Address address,
      multi_address.Address billingAddress,) async {
    isLoading.value = true;
    try {
      selectedAddress = billingAddress;

      List<EstimateShippingMethodModel> data1 = await checkoutOrderAPIRepository.postEstimate(
        address,
      ...
      var carrierCode = (estimatesList.isEmpty)
          ? shippingCarrierCode.value
          : estimatesList[selectedShippingIndex.value].carrierCode ?? kCheckoutShippingCodeFreeShipping;
      var carrierMethodCode = (estimatesList.isEmpty)
          ? shippingMethodCode.value
          : estimatesList[selectedShippingIndex.value].methodCode ?? kCheckoutShippingCodeFreeShipping;

      shipInfoModel.value = await checkoutOrderAPIRepository.postShippingInformation(address, billingAddress,
          multiAddressModel?.value.email ?? '', isSameAsBilling.value, carrierCode, carrierMethodCode);
      await getTotals();
    } on ApiException catch (e) {
      Validators.apiExceptionError(e: e);
    } catch (e) {
  ...
  Future<String> postListForOrder(CartModel? cartlist, String method, paymentId,
      context) async {
    isLoading.value = true;
...
//Creaate Guest Order Api
  Future<String> postGuestOrderForOrder(cartlist, method, paymentId,
      context) async {
    try {
      isLoading.value = true;
      Map podLIst = {
      ...
      String postCreateOrder = await checkoutOrderAPIRepository.postGuestCreateOrderAPIResponse(
        json.encode(podLIst),
  ...
  Future<void> getCountryList() async {
    try {
      isLoading.value = true;
      List<CountryListModel>? data = await addressListAPIRepository.getCountryListResponse();
      if (data?.isNotEmpty == true) {
        getcountryList.value = data!;
        getcountryList.removeWhere((element) => (element.fullNameEnglish?.isEmpty ?? true));
      }
    } on ApiException catch (e) {
      Validators.apiExceptionError(e: e);
    } catch (e) {
...
//CountrList
  Rx<AddressListModal> getAdressList1 = AddressListModal().obs;
  ...
  Future<void> getTotals({bool showDialog = false}) async {
    try {
      ...
      if (getTotalsData.value.getDiscount() != null) {
        ...
        couponCodeController.text = (getTotalsData.value.getDiscountName() ?? '').split('(').last.split(')').first;
      }
      if (showDialog) {
        showAppliedCoupons.value = true;
  ...
  void showDialogForSuccessfullyAppliedCoupons(String couponCode,
      String savedValue) {
    showDialog(
      context: Get.context!,
      builder: (context) {
        ...
        await addressListAPIRepository.addNewAddress(
            ...
            AddAddressRequest(
                region: Region(
                    ...
                    region: selectedState1.value.name == null ? stateController.text : selectedState1.value.name ?? '',
                    regionId: selectedState1.value.name == null ? 12 : int.parse(selectedState1.value.id ?? '')),
                countryId: selectedCoutry1.value.id.toString(),
                street: [(address1Controller.text.toString())],
                firstname: firstNameController.text.toString(),
                lastname: lastNameController.text.toString(),
                telephone: "+$countryCode${phoneNumberController.text.toString()}",
        ...
        if (firstNameBilling.isNotEmpty &&
          ...
          String params1 = jsonEncode({
            "addressInformation": {
              ...
              "billing_address": {
                "region": selectedState2.value.name ?? billingStateController.text,
  ...
  Future<void> placeOrder(CartModel? cartlist, BuildContext context) async {
    ...
    try {
      ...
      if (selectedPaymentIndex.value == 1 || selectedPaymentIndex.value == 3) {
        String code = shipInfoModel.value.paymentMethods?[selectedPaymentIndex.value].code ?? '';
        if (localStore.customerToken.toString() == "") {
          response = await postGuestOrderForOrder(
              cartlist, code, "", context);
        } else {
          response = await postListForOrder(
              cartlist, code, "", context);
        }
